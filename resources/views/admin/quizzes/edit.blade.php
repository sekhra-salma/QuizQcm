    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.quizzes.update", [$quiz->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title" id="title" value="{{ old('title', $quiz->title) }}" required>
                @if($errors->has('title'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('module') ? 'is-invalid' : '' }}" type="text" name="module" id="module" value="{{ old('module', $quiz->module) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nb_qst') ? 'is-invalid' : '' }}" type="number" name="nb_qst" id="nb_qst" value="{{ old('nb_qst', $quiz->nb_qst) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control timepicker {{ $errors->has('time') ? 'is-invalid' : '' }}" type="text" name="time" id="time" value="{{ old('time', $quiz->time) }}" >
                @if($errors->has('time'))
                    <div class="invalid-feedback">
                        {{ $errors->first('time') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('teacher') ? 'is-invalid' : '' }}" name="teacher_id" id="teacher_id" required>
                    @foreach($teachers as $id => $teacher)
                        <option value="{{ $id }}" {{ (old('teacher_id') ? old('teacher_id') : $quiz->teacher->id ?? '') == $id ? 'selected' : '' }}>{{ $teacher }}</option>
                    @endforeach
                </select>
